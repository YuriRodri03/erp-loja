...
export default function Dashboard() {
  ...
  // =========================================================================
  return (
    <div className="font-sans relative max-w-7xl mx-auto space-y-6 sm:space-y-8 p-4 sm:p-6 lg:p-8 overflow-hidden w-full">
      ...
      <div className="bg-white p-5 sm:p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between md:items-center gap-4 w-full">
        ...
        <div className="px-5 py-3 bg-blue-50 text-blue-700 rounded-xl font-bold border border-blue-100 flex items-center justify-center shadow-sm w-full md:w-auto">
          <span className="mr-2 text-xl">🏢</span> <span className="truncate max-w-[200px] sm:max-w-none">{nomeLoja}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 w-full">
        ...
        {/* Menu de Abas (Rolável no Mobile sem quebrar o layout) */}
        <div className="w-full lg:w-max overflow-x-auto pb-2 lg:pb-0 hide-scrollbar rounded-xl">
          <div className="flex space-x-2 bg-gray-100 p-1.5 min-w-max shadow-inner">
            <button onClick={() => setAbaAtiva('geral')} className={`px-4 sm:px-5 py-2.5 rounded-lg font-bold text-sm transition-all whitespace-nowrap ${abaAtiva === 'geral' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Resumo Geral</button>
            <button onClick={() => setAbaAtiva('mensal')} className={`px-4 sm:px-5 py-2.5 rounded-lg font-bold text-sm transition-all whitespace-nowrap ${abaAtiva === 'mensal' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Visão Mensal</button>
            <button onClick={() => setAbaAtiva('anual')} className={`px-4 sm:px-5 py-2.5 rounded-lg font-bold text-sm transition-all whitespace-nowrap ${abaAtiva === 'anual' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Visão Anual</button>
          </div>
        </div>
        ...
        {(abaAtiva === 'mensal' || abaAtiva === 'anual') && (
          <div className="flex flex-col sm:flex-row items-center gap-2 bg-white p-2.5 rounded-xl border border-gray-200 shadow-sm w-full lg:w-auto">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              {abaAtiva === 'mensal' && (
                <select value={mesSelecionado} onChange={(e) => setMesSelecionado(e.target.value)} className="flex-1 sm:flex-none bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 px-3 sm:px-4 py-2 font-bold cursor-pointer transition-colors">
              ...
              <select value={anoSelecionado} onChange={(e) => setAnoSelecionado(e.target.value)} className="flex-1 sm:flex-none bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 px-3 sm:px-4 py-2 font-bold cursor-pointer transition-colors">
      ...
      {abaAtiva === 'geral' && (
        <div className="animate-fade-in space-y-6 sm:space-y-8 w-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 w-full">
            ...
            <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 p-5 sm:p-8 rounded-2xl shadow-lg relative overflow-hidden text-white transition-transform sm:hover:-translate-y-1">
              ...
              <div className="relative z-10">
                ...
                <p className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight mb-2 sm:mb-3 truncate">R$ {lucroLiquido.toFixed(2)}</p>
                <div className="inline-block px-3 py-1.5 bg-indigo-900/50 text-indigo-100 text-xs font-bold rounded-lg backdrop-blur-sm border border-indigo-500/30 truncate max-w-full">
            ...
            <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <p className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight mb-2 sm:mb-3 truncate">R$ {totalEmCaixa.toFixed(2)}</p>
            ...
            <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <p className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight mb-2 sm:mb-3 truncate">R$ {totalDespesas.toFixed(2)}</p>
            ...
            <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <p className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight mb-2 sm:mb-3 truncate">R$ {totalAReceber.toFixed(2)}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 w-full">
            ...
            {/* Alerta de Estoque */}
            <div className="bg-white rounded-2xl p-5 sm:p-8 border border-gray-100 shadow-sm flex flex-col w-full overflow-hidden">
              <div className="flex justify-between items-center mb-5 sm:mb-6 pb-4 border-b border-gray-50">
                ...
                <Link to="/estoque" className="text-xs sm:text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors bg-blue-50 px-3 py-1.5 rounded-lg whitespace-nowrap">Ver Todos</Link>
              </div>
              ...
              {estoqueCritico.length === 0 ? (
                ...
                <div className="space-y-3 w-full">
                  {estoqueCritico.map(prod => (
                    <div key={prod.id} className="flex items-center justify-between p-4 sm:p-5 bg-orange-50/50 rounded-xl border border-orange-100 w-full overflow-hidden">
                      <div className="pr-3 w-[60%]">
                        <p className="font-extrabold text-gray-900 text-sm sm:text-base leading-tight truncate">{prod.nome}</p>
                        <p className="text-xs sm:text-sm font-medium text-gray-500 mt-1 truncate">R$ {prod.preco.toFixed(2)}</p>
                      </div>
                      <div className="text-right w-[40%] flex justify-end">
                        <p className="font-black text-orange-600 text-xl sm:text-2xl truncate">{prod.quantidade} <span className="text-xs font-bold">un</span></p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-2xl p-5 sm:p-8 border border-gray-100 shadow-sm flex flex-col w-full overflow-hidden">
              <div className="flex justify-between items-center mb-5 sm:mb-6 pb-4 border-b border-gray-50">
                ...
                <Link to="/despesas" className="text-xs sm:text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors bg-blue-50 px-3 py-1.5 rounded-lg whitespace-nowrap">Ver Despesas</Link>
              </div>
              ...
              {contasPendentes.length === 0 ? (
                ...
                <div className="space-y-3 w-full">
                  {contasPendentes.map(conta => (
                    <div key={conta.id} className="flex items-center justify-between p-4 sm:p-5 bg-red-50/50 rounded-xl border border-red-100 w-full overflow-hidden">
                      <div className="pr-3 w-[60%]">
                        <p className="font-extrabold text-gray-900 text-sm sm:text-base leading-tight truncate">{conta.descricao}</p>
                        <p className="text-xs font-medium text-gray-500 mt-1 truncate">Venc: {conta.data}</p>
                      </div>
                      <div className="text-right w-[40%] flex justify-end">
                        <p className="font-black text-red-600 text-lg sm:text-xl truncate">R$ {conta.valor.toFixed(2)}</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {abaAtiva === 'mensal' && (
        <div className="animate-fade-in space-y-6 sm:space-y-8 w-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 w-full">
            <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between w-full">
              <div className="w-full truncate pr-4">
                <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest truncate">Entradas no Mês</p>
                <p className="text-2xl sm:text-3xl font-black text-green-600 mt-1 sm:mt-2 tracking-tight truncate">R$ {faturamentoMes.toFixed(2)}</p>
              </div>
              <div className="p-3 sm:p-4 bg-green-50 rounded-xl sm:rounded-2xl flex-shrink-0"><span className="text-2xl sm:text-3xl">💵</span></div>
            </div>
            
            <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between w-full">
              <div className="w-full truncate pr-4">
                <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest truncate">Saídas no Mês</p>
                <p className="text-2xl sm:text-3xl font-black text-red-600 mt-1 sm:mt-2 tracking-tight truncate">R$ {despesasDoMesTotal.toFixed(2)}</p>
              </div>
              <div className="p-3 sm:p-4 bg-red-50 rounded-xl sm:rounded-2xl flex-shrink-0"><span className="text-2xl sm:text-3xl">💸</span></div>
            </div>

            <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between w-full">
              <div className="w-full truncate pr-4">
                <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest truncate">Lucro do Mês</p>
                <p className={`text-2xl sm:text-3xl font-black mt-1 sm:mt-2 tracking-tight truncate ${(faturamentoMes - despesasDoMesTotal) >= 0 ? 'text-indigo-600' : 'text-gray-900'}`}>
                  R$ {(faturamentoMes - despesasDoMesTotal).toFixed(2)}
                </p>
              </div>
              <div className="p-3 sm:p-4 bg-indigo-50 rounded-xl sm:rounded-2xl flex-shrink-0"><span className="text-2xl sm:text-3xl">📈</span></div>
            </div>
          </div>

          {/* Gráfico Principal */}
          <div className="bg-white p-3 sm:p-8 rounded-2xl border border-gray-100 shadow-sm w-full overflow-hidden">
            <div className="mb-4 sm:mb-8 text-center sm:text-left px-2 sm:px-0">
            ...
            <div className="h-[250px] sm:h-[400px] w-full relative">
              <div className="absolute inset-0">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={dadosMensais} margin={{ top: 10, right: 0, left: -25, bottom: 0 }}>
                    <defs>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 w-full">
            ...
            <div className="bg-white p-4 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex flex-col w-full overflow-hidden">
              <h3 className="text-lg sm:text-xl font-extrabold text-gray-900 mb-4 sm:mb-6 text-center lg:text-left">Top Produtos</h3>
              <div className="flex-1 w-full min-h-[250px] sm:min-h-[300px] relative">
                {topProdutosMes.length > 0 ? (
                  <div className="absolute inset-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={topProdutosMes} layout="vertical" margin={{ top: 0, right: 10, left: 0, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="#f1f5f9" />
                        <XAxis type="number" tick={{fill: '#64748b', fontSize: 10, fontWeight: 'bold'}} axisLine={false} tickLine={false} />
                        <YAxis dataKey="name" type="category" tick={{fill: '#334155', fontSize: 10, fontWeight: 'bold'}} axisLine={false} tickLine={false} width={80} />
            ...
            <div className="bg-white p-4 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex flex-col w-full overflow-hidden">
              <h3 className="text-lg sm:text-xl font-extrabold text-gray-900 mb-4 sm:mb-6 text-center lg:text-left">Top Clientes</h3>
              {topClientesMes.length > 0 ? (
                <div className="flex flex-col space-y-3 flex-1 justify-center w-full">
                  {topClientesMes.map((cli, idx) => (
                    <div key={idx} className="flex justify-between items-center p-3 hover:bg-gray-50 rounded-xl transition-colors w-full overflow-hidden">
                      <div className="flex items-center w-[60%]">
                        <span className="w-8 h-8 rounded-lg bg-blue-50 text-blue-700 flex items-center justify-center text-xs font-black mr-3 shadow-sm flex-shrink-0">{idx + 1}</span>
                        <span className="font-bold text-gray-800 text-sm sm:text-base truncate">{cli.nome}</span>
                      </div>
                      <span className="font-black text-green-600 text-sm sm:text-lg w-[40%] text-right truncate">R$ {cli.totalGasto.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              ) : <div className="h-full flex items-center justify-center text-gray-400 font-medium text-sm">Sem dados no mês.</div>}
            </div>
          </div>
        </div>
      )}
      ...
      {abaAtiva === 'anual' && (
        <div className="animate-fade-in space-y-6 sm:space-y-8 w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 w-full">
            <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between w-full">
              <div className="w-full truncate pr-4">
                <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest truncate">Entradas do Ano</p>
                <p className="text-2xl sm:text-3xl font-black text-green-600 mt-1 sm:mt-2 tracking-tight truncate">R$ {faturamentoAno.toFixed(2)}</p>
              </div>
              <div className="p-3 sm:p-4 bg-green-50 rounded-xl sm:rounded-2xl flex-shrink-0"><span className="text-2xl sm:text-3xl">💵</span></div>
            </div>
            ...
            <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between w-full">
              <div className="w-full truncate pr-4">
                <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest truncate">Saídas do Ano</p>
                <p className="text-2xl sm:text-3xl font-black text-red-600 mt-1 sm:mt-2 tracking-tight truncate">R$ {despesasDoAnoTotal.toFixed(2)}</p>
              </div>
              <div className="p-3 sm:p-4 bg-red-50 rounded-xl sm:rounded-2xl flex-shrink-0"><span className="text-2xl sm:text-3xl">💸</span></div>
            </div>
            ...
            <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between w-full">
              <div className="w-full truncate pr-4">
                <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest truncate">Lucro Anual</p>
                <p className={`text-2xl sm:text-3xl font-black mt-1 sm:mt-2 tracking-tight truncate ${(faturamentoAno - despesasDoAnoTotal) >= 0 ? 'text-indigo-600' : 'text-gray-900'}`}>
          ...
          <div className="bg-white p-3 sm:p-8 rounded-2xl border border-gray-100 shadow-sm w-full overflow-hidden">
            ...
            <div className="h-[250px] sm:h-[400px] w-full relative">
              <div className="absolute inset-0">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={dadosAnuais} margin={{ top: 10, right: 0, left: -25, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                    <XAxis dataKey="name" tick={{fill: '#64748b', fontSize: 10, fontWeight: 'bold'}} axisLine={false} tickLine={false} dy={10} />
                    <YAxis tick={{fill: '#64748b', fontSize: 10, fontWeight: 'bold'}} axisLine={false} tickLine={false} tickFormatter={(value) => `R$ ${value}`} />
                    ...
                    <Legend iconType="circle" wrapperStyle={{ paddingTop: '15px', fontSize: '10px' }}/>
                    <Bar dataKey="Caixa" name="Entradas" fill="#22c55e" radius={[4, 4, 0, 0]} barSize={15} />
                    <Bar dataKey="Despesas" name="Saídas" fill="#ef4444" radius={[4, 4, 0, 0]} barSize={15} />
                    <Line type="monotone" dataKey="Lucro" name="Lucro" stroke="#6366f1" strokeWidth={3} dot={{r: 3, strokeWidth: 2, fill: '#fff', stroke: '#6366f1'}} activeDot={{r: 6}} />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 w-full">
            <div className="bg-white p-4 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center w-full overflow-hidden">
